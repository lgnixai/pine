...
#[derive(Clone, Debug, PartialEq)]
pub enum Expression {
    Infix(Box<ExprInfix>),//中缀
    Prefix(Box<ExprPrefix>), //前缀
    Assign(Box<ExprAssign>), //分配，赋值
    ArrayAccessAssign(Box<ExprArrayAccessAssign>),// 分配数组
    Call(Box<ExprCall>),  //执行 调用
    Literal(ExprLiteral), //原子
    Var(ExprVar),      //定义变量
    Get(Box<ExprGet>),
    Set(Box<ExprSet>),
    Super(ExprSuper),
